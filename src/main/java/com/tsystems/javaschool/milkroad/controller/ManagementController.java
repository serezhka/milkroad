...
import com.tsystems.javaschool.milkroad.controller.converter.EnumConverter;
...
public class ManagementController {
    ...
    public ModelAndView updateOrder(
            @ModelAttribute("orderID") final Long orderID,
            @ModelAttribute("paymentMethod") final PaymentMethodEnum paymentMethodEnum,
            @ModelAttribute("paymentStatus") final PaymentStatusEnum paymentStatusEnum,
            @ModelAttribute("shippingMethod") final ShippingMethodEnum shippingMethodEnum,
            @ModelAttribute("shippingStatus") final ShippingStatusEnum shippingStatusEnum,
            @RequestParam(value = "returnType", required = false, defaultValue = "jsp") final String returnType) {
        final OrderDTO orderDTO = new OrderDTO();
        orderDTO.setId(orderID);
        orderDTO.setPaymentMethod(paymentMethodEnum);
        orderDTO.setPaymentStatus(paymentStatusEnum);
        orderDTO.setShippingMethod(shippingMethodEnum);
        orderDTO.setShippingStatus(shippingStatusEnum);
        try {
            orderService.updateOrder(orderDTO);
        } catch (final MilkroadServiceException e) {
            return (returnType.equalsIgnoreCase("JSON")
                    ? new ModelAndView(new MappingJackson2JsonView())
                    : new ModelAndView("single-message"))
                    .addObject("message", "DB error! Please, try later");
        }
        return (returnType.equalsIgnoreCase("JSON")
                ? new ModelAndView(new MappingJackson2JsonView())
                : new ModelAndView("redirect:/management/editOrders"));
    }
